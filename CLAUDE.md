...
| `docs/` | Rules shared by every case. `writing-rules.md` — the two voices (page copy vs. research notes), what a note must record, bilingual. `references.md` — external projects borrowed from, with licenses. `deploy.md` — why every case ships as a static export, and where to host it. _(UI system and provenance still to be extracted from the reference case.)_ | yes |
